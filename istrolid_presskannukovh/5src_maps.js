...
(function () {
    var chooseNumber, chooseOne, genBox, genCTF, genClouds, genDebris, genDodads,
        genFFA, genIO, genRocks, genSurvival, genSymmetrical, genTTT;
    ...
    mapping.green = {
        rockColor: [33, 33, 33, 255],
        spotColor: [67, 160, 71, 10],
        fillColor: [135, 205, 142, 255]
    };

    mapping.purpleyellow = {
        rockColor: [0, 105, 92, 255],
        spotColor: [94, 53, 177, 255],
        fillColor: [255, 235, 59, 100]
    };

    mapping.blackred = {
        rockColor: [255, 234, 0, 255],
        spotColor: [15, 15, 15, 255],
        fillColor: [200, 0, 0, 255]
    };

    mapping.greenorange = {
        rockColor: [255, 234, 0, 255],
        spotColor: [27, 94, 32, 255],
    ...
    mapping.bluepurple = {
        rockColor: [255, 234, 0, 255],
        spotColor: [1, 87, 155, 255],
        fillColor: [216, 27, 96, 255]
    };

    mapping.themes = [mapping.main,
        mapping.green,
        mapping.purpleyellow,
        mapping.ghostly,
        mapping.grayblue,
    ...
    mapping.generate = function (seed) {
        ...
        if (sim.makeRocks) {
            fns = shuffle([genClouds, genDebris, genRocks, genDodads]);
            r = mr.random();
            if (r < .2) {
                fns.pop()();
                fns.pop()();
    ...
    mapping.genTower = function () {
        let _, blocks, i, ref, results, spec, thing, u;
        ref = sim.things;
        results = [];
        for (_ in ref) {
            thing = ref[_];
    ...
    genIO = function () {
        let _, a, i, len, m, o, p, pos, q, r, ref, ref1, ref2, results, s, t, tooClose, w;
        for (i = m = 0; m < 4; i = ++m) {
    ...
    genSymmetrical = function () {
        let _, a, b, from_center, i, m, mainSpawn, o, pos, ref, ref1, results, t, tooClose;
        mainSpawn = a = new types.SpawnPoint();
        a.side = "alpha";
        a.spawn = "alpha";
    ...
    genSurvival = function () {
        let cp, i, m, ref, results, spawn, th;
    ...
    genCTF = function () {
        let _, a, alphaSpawn, b, betaSpawn, createFlag, flag, from_center, homePoints, i, len, m, o, point, pos, q, ref,
        ...
        createFlag = function () {
            ...
            flag.tick = function () {
                let closest, eHome, eHomeDist, enemySpawn, home, homeDist, minDist, ref, ref1, ref2, ref3, spawn;
    ...
    genTTT = function () {
        let alpha, beta, i, m, mu, o, q, results, u, x, y;
        for (x = m = -1; m <= 1; x = ++m) {
            for (y = o = -1; o <= 1; y = ++o) {
    ...
    genFFA = function () {
        let cp, i, m, makePoint, o, p, player, players, ref, ref1, results, sp, th;
        makePoint = function (r, th, side, type) {
            let point;
            point = new type();
            point.z = -.01;
            point.pos[0] = Math.cos(th) * r * sim.mapScale;
        ...
        for (i = m = 0, ref = players.length; 0 <= ref ? m < ref : m > ref; i = 0 <= ref ? ++m : --m) {
            ...
            player = players[i];
            sp = makePoint(1800, th, player.side, types.SpawnPoint);
            sp.spawn = player.side;
            cp = makePoint(1800, th, player.side, types.CommandPoint);
            cp.linkedSpawn = sp;
            cp.radius = sp.radius;
            cp.size = [cp.radius / 250, cp.radius / 250];
            players[i].usingSpawn = sp;
        }
        cp = makePoint(0, 0, "neutral", types.CommandPoint);
        cp.radius = 300;
        cp.size = [cp.radius / 250, cp.radius / 250];
        cp.value = 1.2;
        results = [];
        for (i = o = 0, ref1 = sim.numComPoints - 1; 0 <= ref1 ? o < ref1 : o > ref1; i = 0 <= ref1 ? ++o : --o) {
            th = i * Math.PI * 2 / (sim.numComPoints - 1);
            if (i % 2 === 0) {
                results.push(makePoint(640, th, "neutral", types.CommandPoint));
            } else {
                cp = makePoint(880, th, "neutral", types.CommandPoint);
                cp.radius = 200;
                cp.size = [cp.radius / 250, cp.radius / 250];
                results.push(cp.value = 0.8);
            }
        }
        return results;
    };
    ...
    genDebris = function () {
        let c, clusterCenter, debri, debriColor, i, m, n, ref, results;
        debriColor = sim.theme.spotColor;
        ...
        for (c = m = 0, ref = n * mr.random(); 0 <= ref ? m < ref : m > ref; c = 0 <= ref ? ++m : --m) {
            ...
            if (mr.random() < .7) {
                debri = new types.Rock();
                if (mr.random() > .2) {
                    debri.image = "img/debree/bigdebree" + (chooseNumber(12)) + ".png";
                } else {
                    debri.image = "img/debree/civ" + (chooseNumber(5)) + ".png";
                }
                v2.add(debri.pos, clusterCenter);
                debri.z = (mr.random() - .5) * 200;
                debri.color = debriColor;
                debri.rot = mr.random() * 2 * Math.PI;
                sim.things[debri.id] = debri;
            }
            results.push((function () {
                let o, ref1, results1;
                results1 = [];
                for (i = o = 0, ref1 = 20 * mr.random(); 0 <= ref1 ? o < ref1 : o > ref1; i = 0 <= ref1 ? ++o : --o) {
                    debri = new types.Rock();
                    debri.image = "img/debree/debree" + (chooseNumber(25)) + ".png";
                    randomVector(debri.pos);
                    v2.scale(debri.pos, mr.random() * 600);
                    v2.add(debri.pos, clusterCenter);
                    debri.z = (mr.random() - .5) * 200;
                    debri.color = debriColor;
                    debri.rot = mr.random() * 2 * Math.PI;
                    results1.push(sim.things[debri.id] = debri);
                }
                return results1;
            })())
        }
    ...
    genDodads = function () {
        let _, chooseDodad, dodadColor, genRing, ref, results, single, thing;
        ...
        genRing = (function (_this) {
            return function (pos, radius) {
                let dodad, i, image, level, m, ref, results, z;
                image = chooseDodad();
                z = -mr.random() * 6 - 3;
                results = [];
    ...
    mapping.save = function (name) {
        let k, len, m, map, n, num, p, player, ref, ref1, ref2, spec, thing;
        p = function (pos) {
    ...
    mapping.load = function (name) {
        let base, id, len, m, map, owner, p, player, ref, ref1, results, thing;
