...
return
Interpolator;
    private hasProp = {}.hasOwnProperty;
    private diffVec = v2.create(null);
    private sound = true;
    private fast = false;
    private smoothFactor: number;
    private fastParticles: any;
    private things: {};
    private firstUpdate: boolean;
    private step: number;
    private players: Player[];
    private particles: {};
    private winningSide: null;
    private avgFrame: number;
    private stepTime: number;
    private lastFrame: number;
    private t: number;
    private avgDt: number;
    private avgTime: number;
    private allMessages: any[];
    private dataQ: any[];
    private wait: number;
    private prevWait: number;
    ...
    constructor () {
        this.think.prototype.bind(this);
        this.step = 0;
        this.firstUpdate = true;
        this.things = {};
        this.players = Sim.Instance.players;
        this.particles = {};
        this.fastParticles = {};
        this.avgFrame = 9;
        this.lastFrame = 0;
        this.stepTime = Utils.now();
        this.t = Utils.now();
        this.avgDt = 1 / 60;
    ...
    static drawThingsList () {
        let _, particle, ref, ref1, thing, things;
        things = [];
    ...
    gameStarted () {
        ...
        track("start");
        if (!Sim.Instance.galaxyStar && !Sim.Instance.local) {
    ...
    draw () {
        let ai, color, i, j, l, len, len1, len2, len3, m, o, p, particles, player, q, ref, ref1, ref2, thing,
            things;
        this.advance();
        ref = Sim.Instance.ais;
        for (j = 0, len = ref.length; j < len; j++) {
            ai = ref[j];
            ai.draw();
        }

        things = Interpolator.drawThingsList();
    ...
    uploadReplay () {
        ...
        if (this.replay === "recording") {
            this.replay = "off";
            data = JSON.stringify((function (passed_this) {
                let j, len, ref, results;
                ref = passed_this.replayFrames;
                results = [];
                for (j = 0, len = ref.length; j < len; j++) {
                    frame = ref[j];
    ...
    process (data) {
        ...
        if (intp.countDown === 5 * 16 && !Sim.Instance.local) {
            onecup.refresh();
        }
                ...
                for (q = 0, len3 = t.length; q < len3; q++) {
                    ...
                    switch (k) {
                        ...
                        case "orders":
                            thing.orders = v;
                            if (thing.preOrders) {
                                thing.preOrders = thing.preOrders.filter(function (order: { step: number; }) {
                                    return order.step + 16 * 5 < Sim.Instance.step;
                                });
                            }
                            break;
                        default:
