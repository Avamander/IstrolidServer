...
(function () {
    var findRank, allowedSim, checkHost, checkRunning, defStats, helpMessage, parseStat,
    ...
    let find_best_teams = function (players) {
        ...
        for (let i = 0; i < combinations.length; i++){
            combination_score = Math.abs(
                (findRank(combinations[i][0].name) + findRank(combinations[i][1].name) + findRank(combinations[i][2].name))
                -
                (findRank(combinations[i][3].name) + findRank(combinations[i][4].name) + findRank(combinations[i][5].name))
            );
            if(best_combination_score > combination_score){
    ...
    let get_balance_index = function () {
        ...
        let beta_sum = 0;
        for (let player in beta_players) {
            beta_sum += findRank(player.name);
        }

        let alpha_sum = 0;
        for (let player in alpha_players) {
            alpha_sum += findRank(player.name);
        }
